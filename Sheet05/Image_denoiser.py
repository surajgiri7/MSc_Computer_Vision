...
def grayscale_img_denoiser(I, rho=0.6):
    ...
    for _ in range(max_iter):
        improved = False
        for alpha in labels:
            energy, Denoised_I = calculate_energy(alpha, I, Denoised_I, rho)
...
def calculate_energy(alpha, I, Denoised_I, rho):
    ...
    D = np.abs(I.reshape(I.shape + (1,)) - np.array([alpha]).reshape((1, 1, -1)))
    D = np.where(D == 0, rho, (1 - rho) / 2)

    for r in range(row):
        for c in range(col):
            add_t_edge(g, nodeids, Denoised_I, label_dict, V, alpha, r, c, rho, D)
...
def add_t_edge(g, nodeids, Denoised_I, label_dict, V, alpha, r, c, rho, D):
    curr_state = Denoised_I[r, c]
    if curr_state == alpha:
        g.add_tedge(nodeids[r, c], D[r, c, label_dict[alpha]], np.inf)
        if r < Denoised_I.shape[0] - 1 and Denoised_I[r + 1, c] != alpha:
        ...
        if r < Denoised_I.shape[0] - 1 and Denoised_I[r + 1, c] == alpha:
            ...
            if r < Denoised_I.shape[0] - 1 and Denoised_I[r, c] != Denoised_I[r + 1, c]:
